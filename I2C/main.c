...
void display_task(void *data)
{
  ...
  while(1)
  {
    ...
    {
      printf("Telemetry Received -> Boiler Temp: %.1f C\n", telemetry);
      ...
      if(telemetry > 120.0f) 
      {
        ...
        for(int i = 0; i < 256; i++) 
        {
          i2c_master_write_byte(command_link, 0xFF, true); 
        }
      }
      else
      {
        for(int i = 0; i < 256; i++) 
        {
          i2c_master_write_byte(command_link, 0x00, true); 
        }
      }
...
void oled_hardware_init(void)
{
    ...
    i2c_master_write_byte(cmd, (OLED << 1) | I2C_MASTER_WRITE, true);
